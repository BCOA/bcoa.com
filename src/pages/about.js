...
const Member = ({ member }) => (
  <div
    className={`${
      member.principal
        ? "principal bp-2_marginBottom-33"
        : "bp-2_marginBottom-13"
    } `}
  >
    ...
    <p className={`${member.principal ? "f-copy-bold" : ""}  marginBottom-5`}>
      {member.principalInfo}
    </p>
    {member.principal && (
      <p className="f-copy marginBottom-13">{member.description}</p>
    )}
  </div>
);
    ...
    {collaborator.url ? (
      <a href={collaborator.url} target="_blank" rel="noopener noreferrer">
        ...
        {collaborator.description && (
          <p className="marginBottom-7">{collaborator.description}</p>
        )}
      </a>
    ) : (
      <div>
